...
| 340: http://openjdk.java.net/jeps/340[One AArch64 Port, Not Two] | 只保留一个 AArch64 实现

| 341: http://openjdk.java.net/jeps/341[Default CDS Archives] | 默认类数据共享(CDS)存档

| 344: http://openjdk.java.net/jeps/344[Abortable Mixed Collections for G1] | 可中止的 G1 Mixed GC

| 346: http://openjdk.java.net/jeps/346[Promptly Return Unused Committed Memory from G1] | G1 及时返回未使用的已分配内存
|===
...
image::{oss-images}/java12/java-12-2.png[]
...
image::{oss-images}/java12/java-12-1.png[]
...
* JDK 5 引入了 Class-Data Sharing 可以用于多个 JVM 共享 class,提升启动速度,最早只支持 system classes 及 serial GC.
* JDK 9 对其进行扩展以支持 application classes 及其他GC算法.
* java 10 的新特性 JEP 310: Application Class-Data Sharing 扩展了 JDK5 引入的 Class-Data Sharing,支持 application 的 Class-Data Sharing 并开源出来(以前是 commercial feature)
...
* JDK 11 将 `-Xshare:off` 改为默认 `-Xshare:auto`,以更加方便使用CDS特性
...
JDK 12 之前,想要利用 CDS 的用户,即使仅使用JDK中提供的默认类列表,也必须 `java -Xshare:dump` 作为额外的步骤来运行.
...
.Java 12
----
include::{rootProject}/java12/src/main/java/com/jcohy/sample/java12/StringTest.java[tag=transform]
----
...
.Java 12
----
include::{rootProject}/java12/src/main/java/com/jcohy/sample/java12/StringTest.java[tag=transform2]
----
...
.Java 12
----
include::{rootProject}/java12/src/main/java/com/jcohy/sample/java12/StringTest.java[tag=transform3]
----
...
.Java 12
----
include::{rootProject}/java12/src/main/java/com/jcohy/sample/java12/StringTest.java[tag=indent]
----
...
从 Java 12 开始，String 类实现了两个额外的接口 `java.lang.constant.Constable` 和 `java.lang.constant.ConstantDesc`。

==== String 常量

String 类还引入了两个额外的方法 `describeConstable()` 和 `resolveConstantDesc(MethodHandles.Lookup)`。

它们是底层 API，用于提供字节码解析和生成功能的库和工具，例如 Byte Buddy。

请注意，`Constable` 类型的值是可以在 Java 类文件的常量池中表示的常量，如 https://docs.oracle.com/javase/specs/jvms/se7/html/jvms-4.html#jvms-4.4[JVMS 4.4] 中所述，并且其实例可以名义上将自己描述为 `ConstantDesc`。

`resolveConstantDesc()` 与 `describeConstable()` 类似，不同之处在于此方法返回的是 `ConstantDesc` 的实例。
...
.Java 12
----
include::{rootProject}/java12/src/main/java/com/jcohy/sample/java12/FileTest.java[tag=mismatch]
----

[[java-12-feature-grama-collectors]]
=== Collectors.teeing()
...
.Java 12
----
include::{rootProject}/java12/src/main/java/com/jcohy/sample/java12/StreamApi.java[tag=teeing]
----
